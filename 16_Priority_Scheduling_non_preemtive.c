...
int main()
{
    ...
    while (completed < n)
    {
        ...
        // Select highest priority among arrived processes (NON-preemptive)
        for (int i = 0; i < n; i++)
        {
            if (at[i] <= current_time && is_completed[i] == 0)
            {
                ...
                // tie-breaker: if same priority, choose earlier arrival
                else if (pr[i] == best_pr)
                {
                    ...
                    // if still tie, choose smaller PID
